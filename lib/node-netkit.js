...
var nfevents = require('../nf/nfevents.js');
...
var route = require('../ip/route.js');
var link = require('../ip/link.js');
var netev = require('../ip/netevent.js');
var iw = require('../iw/iwcommand.js');
...
var extendthis = {
	...
	open: function() {
		debug("open(): " + util.inspect(this));
		var ret = this._open();
		if(this.fd < 1 || !ret) {
			error("TUN device not created or valid.");
			return false;
		} else {
			if(!this.stream) {
	...
	// 	} else {
	// 		error("Socket for TUN device not created. Did you call open()?");
	// 	}
	// },
...
TunInterfaceStream.prototype._doRead = function(size) {
	...
	this.IF._getData(function(buf,readsize,error){
		...
		if(error && error.errno > 0) {
			error("Error ocurred: " + JSON.stringify(error));
		}
	}, size);
}
...
TunInterfaceStream.prototype._write = function(chunk,encoding,callback) {
	if(typeof chunk == 'string') {
		...
		if(Buffer.isBuffer(chunk)) {
			...
			error("Passed a non Buffer or string to TUN._write");
	}
};
...
var boundNeighbor = neigh.neighbor;
boundNeighbor.bind(this);
var boundRoute = route.route;
boundRoute.bind(this);
...
var boundNfCommand = nfcommand.command;
boundNfCommand.bind(this);
var boundNfEvent = nfcommand.on;
boundNfEvent.bind(this);
var boundNfCommandByStructure = nfcommand.command_by_structure;
boundNfCommandByStructure.bind(this);
var boundNfEvents = nfevents.listen;
boundNfEvents.bind(this);

var boundIwCommand = iw.command;
boundIwCommand.bind(this);
...
var nk = {
	...
	addNameServers: cmn.nativelib.addNameServers,
	...
	nnf: boundNfCommand,
	nnfEvent: boundNfEvent,
	nnfApply: boundNfCommandByStructure,
	nfListen: boundNfEvents,

	iw: boundIwCommand,


	assignDebugLogger: function(func) {
		cmn.logger.set_debug_logger(func);
	},
	assignErrorLogger: function(func) {
		cmn.logger.set_error_logger(func);
	},
...
/**
 * NETLINK_NETFILTER related constants and functions
 */
var nf = nk.nf = require('../nl/nfnetlink.js');
nk.nf.parser = require('../nf/node-netfilter.js');
